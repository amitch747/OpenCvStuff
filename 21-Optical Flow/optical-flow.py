...
def lucasKnade():
    videoCapObj = cv.VideoCapture(0)
    if not videoCapObj.isOpened():
         exit()
    
    # Setup parameters
    shiTomasiCornerParams = dict(maxCorners=20, qualityLevel=0.3, minDistance=50, blockSize=7)
    luncasKanadeParams = dict(winSize = (15, 15), maxLevel=2, criteria=(cv.TERM_CRITERIA_EPS|cv.TERM_CRITERIA_COUNT, 10,0.03))
    ...
    while True:
        ...
        cornersCur, foundStatus,_ = cv.calcOpticalFlowPyrLK(frameGrayPrev, frameGrayCur, cornersPrev,None,**luncasKanadeParams)
...
def denseOpticalFlow(): 
    ...
    while True:
        ...
        flow = cv.calcOpticalFlowFarneback(prev=imgPrev, next=imgCur, flow=None, pyr_scale=0.5,
